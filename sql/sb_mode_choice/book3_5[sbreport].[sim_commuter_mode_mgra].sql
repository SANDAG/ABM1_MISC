...
/****** Object:  StoredProcedure [sbreport].[sim_commuter_mode_mgra]    Script Date: 11/20/2019 3:30:18 PM ******/
SET ANSI_NULLS ON
GO
...
select  scenario_id
       ...
       ,sum(isnull([SOV],0)+isnull([HOV2],0)+isnull([HOV3],0)+isnull([WALK],0)+isnull([BIKE],0)+isnull([TRANSIT],0)+isnull([SBUS],0)+isnull([TAXI],0)+isnull([AIR],0)) as TotalCommuterTrips
from t1
...
select @scenario_id as scenario_id
       ...
       ,isnull([SOV],0)+isnull([HOV2],0)+isnull([HOV3],0)+isnull([WALK],0)+isnull([BIKE],0)+isnull([TRANSIT],0)+isnull([SBUS],0)+isnull([TAXI],0)+isnull([AIR],0) as TotalCommuterTrips
from t1
